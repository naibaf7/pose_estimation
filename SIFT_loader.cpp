...
void SIFT_loader::load_features(const char *filename, SIFT_FORMAT format) {
	...
	if (!loaded)
		std::cerr << "Could not load the features from the given file "
				<< filename << std::endl;
}
...
bool SIFT_loader::save_features_lowe(const char *filename) {
	...
	for (uint32_t i = 0; i < mNbFeatures; ++i) {
		outstream << mKeypoints[i].y << " " << mKeypoints[i].x << " "
				<< mKeypoints[i].scale << " " << mKeypoints[i].orientation
				<< std::endl;
